function help_modal(column) {
    ...
    switch (column) {
        ...
        case 1:
            ...
            $('#gen-modal-body').html("In this section there are three modes. You can 'edit' which lets you color individual cells on the stamps. 'Flood fill' lets you flood fill the stamp with a color. 'Copy' lets you copy cells from the 'Task and Solution Section' and paste it into a stamp (press 'v' after selecting the cell to paste into) or create a new stamp (press 'n').<br/><br/>Below this mode-selecting section, there are a list of buttons to edit stamps. 'Create' adds a new stamp of the entered size, 'remove' deletes the selected stamp, 'duplicate' creates a copy of the selected stamp, 'rotate' rotates the selected stamp counter clockwise, 'flip' flips the stamp vertically, 'scale' loops through scaling the selected stamp 1x,2x,3x,4x, and 5x. Below is a display of all created stamps. To select a stamp, press the green label above it.<br/><br/>Keep in mind that white cells represent transparent/empty, and empty stamps are still sent to the database (so delete them).");
            break;
        case 2:
